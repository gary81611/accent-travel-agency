import { getImageUrl } from "@/lib/supabase/storage";
import { getSiteImages } from "@/lib/supabase/site-images";
import { getSiteSettings } from "@/lib/supabase/site-settings";
import Image from "next/image";
import ContactForm from "./ContactForm";

export const metadata = { title: "Contact Us" };

export default async function ContactPage() {
  const [img, settings] = await Promise.all([getSiteImages(), getSiteSettings()]);
  const phoneRaw = settings.phone.replace(/\D/g, "");

  return (
    <>
      {/* Hero */}
      <section className="relative h-80 flex items-center justify-center overflow-hidden pt-20">
        <Image
          src={getImageUrl(img.hero_contact || "heroes/contact-hero-2880w.jpg")}
          alt={`Contact ${settings.business_name}`}
          fill
          className="object-cover"
          style={{ objectPosition: img.hero_contact_pos || "center center" }}
          priority
        />
        <div className="absolute inset-0 bg-brand-teal/60" />
        <div className="relative z-10 text-center text-white">
          <h1 className="font-[family-name:var(--font-heading)] text-4xl md:text-6xl font-bold uppercase tracking-wider">
            Contact Us
          </h1>
        </div>
      </section>

      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {/* Contact form */}
          <div>
            <h2 className="font-[family-name:var(--font-heading)] text-2xl md:text-3xl text-brand-teal uppercase mb-2">
              Send Us a Message
            </h2>
            <div className="w-16 h-1 bg-brand-gold mb-4" />
            <p className="text-brand-charcoal mb-6">
              Your destination waits! For all of your travel needs, contact our experienced and
              reliable staff. If we miss your call for any reason, we&apos;ll call you back the same
              business day.
            </p>
            <ContactForm email={settings.email} />
          </div>

          {/* Contact info */}
          <div>
            <h2 className="font-[family-name:var(--font-heading)] text-2xl md:text-3xl text-brand-teal uppercase mb-2">
              Get in Touch
            </h2>
            <div className="w-16 h-1 bg-brand-gold mb-6" />

            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="bg-brand-gold rounded-full p-3 text-white shrink-0">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                </div>
                <div>
                  <h3 className="font-semibold text-brand-charcoal">Call or Text</h3>
                  <a href={`tel:${phoneRaw}`} className="text-brand-gold text-lg font-bold hover:underline">
                    {settings.phone}
                  </a>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="bg-brand-gold rounded-full p-3 text-white shrink-0">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <div>
                  <h3 className="font-semibold text-brand-charcoal">Email</h3>
                  <a href={`mailto:${settings.email}`} className="text-brand-gold hover:underline">
                    {settings.email}
                  </a>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="bg-brand-gold rounded-full p-3 text-white shrink-0">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                </div>
                <div>
                  <h3 className="font-semibold text-brand-charcoal">Address</h3>
                  <p className="text-brand-charcoal">
                    {settings.address}<br />
                    {settings.city}, {settings.state} {settings.zip}
                  </p>
                </div>
              </div>

              {settings.facebook && (
                <div className="flex items-start gap-4">
                  <div className="bg-brand-gold rounded-full p-3 text-white shrink-0">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  </div>
                  <div>
                    <h3 className="font-semibold text-brand-charcoal">Follow Us</h3>
                    <a
                      href={settings.facebook}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-brand-gold hover:underline"
                    >
                      {settings.business_name} on Facebook
                    </a>
                  </div>
                </div>
              )}

            </div>
          </div>
        </div>
      </div>
    </>
  );
}
